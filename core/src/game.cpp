								   ...
								   Item *item, uint32_t count, Item **_moveItem, uint32_t flags /*= 0*/, Creature *actor /* = nullptr*/, Item *tradeItem /* = nullptr*/, const Position *fromPos /*= nullptr*/, const Position *toPos /*= nullptr*/)
{
	...
	if (toTile != nullptr && !houseTile)
	{
		// If the parent cylinder is a tile and has no parent itself, it's a ground tile
		// g_game.internalRemoveItem(item);
		return RETURNVALUE_NOTPOSSIBLE; // or RETURNVALUE_NOERROR if destroying
	}									//@Drop to ground system
