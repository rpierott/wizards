							  ...
							  uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	Container *containerForTrade = tradeItem->getContainer();
	if (!containerForTrade) // && !tradeItem)
	{
		return;
	}

	std::vector<const Container *> containers{containerForTrade};
	size_t i = 0;
	...
	while (i < containers.size())
	{
		...
		for (Item *item : container->getItemList())
		{
			...
			else
			{
				if (item->isSoulbound())
				{
					player->sendCancelMessage(RETURNVALUE_CANNOTDROPSOULBOUND);
					return;
				}
			}
		}
