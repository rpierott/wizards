...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream &propStream)
{
	...
	{
		uint8_t soulbound;
		if (!propStream.read<uint8_t>(soulbound))
		{
			return ATTR_READ_ERROR;
		}

		setIntAttr(ITEM_ATTRIBUTE_SOULBOUND, soulbound);
		break;
	} //@Soulbound system
...
void Item::serializeAttr(PropWriteStream &propWriteStream) const
{
	...
	{
		propWriteStream.write<uint8_t>(ATTR_SOULBOUND);
		propWriteStream.write<uint8_t>(getIntAttr(ITEM_ATTRIBUTE_SOULBOUND));
	} // Soulbound system
