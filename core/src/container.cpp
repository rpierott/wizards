								...
								uint32_t flags, Creature *actor /* = nullptr*/) const
{
	...
	// soulbound items can be only moved into depot chest, bags or player slots
	if (item->isSoulbound() && !dynamic_cast<const DepotChest *>(this) && !dynamic_cast<const Container *>(this))
	{
		return RETURNVALUE_ITEMCANNOTBEMOVEDTHERE;
	} //@Soulbound system
